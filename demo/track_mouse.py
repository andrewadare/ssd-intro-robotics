...
class MouseTracker:
    ...
    def reset_kf(self, x, y):
        self.kf = point_2d_kalman_filter(
            [x, 0.0, y, 0.0], self.sigma_proc, self.sigma_meas
        )
    ...
    def on_right_click(self, *args):
        ...
        self.sigma_proc = self.map_linear(mid, hi, lo, 1e-6, self.max_sigma_proc)
        self.sigma_meas = self.map_linear(mid, hi, lo, 1e-6, self.max_sigma_meas)
    ...
    def on_sigma_proc_change(self, val):
        ...
        std = self.map_linear(val, *self.slider_range, 1e-6, self.max_sigma_proc)
        self.sigma_proc = std
    ...
    def on_sigma_meas_change(self, val):
        ...
        std = self.map_linear(val, *self.slider_range, 1e-6, self.max_sigma_meas)
        self.sigma_meas = std
        if self.kf is not None:
    ...
    def draw_ellipse(
        self, x, y, a, b, phi, color=(255, 255, 255), nsigma=3.0, scale=(1.0, 1.0)
    ):
        center = [int(s * x) for (s, x) in zip(scale, (x, y))]
        axes = [int(np.abs(s) * x) for (s, x) in zip(scale, (nsigma * a, nsigma * b))]
        angle = int(phi * 180 / np.pi)
        cv2.ellipse(self.im, tuple(center), tuple(axes), angle, 0, 360, color, 2)
    ...
    def draw(self):
        ...
        cv2.putText(
            self.im,
            "process noise: {:.2g}".format(self.sigma_proc),
            (w - 200, 20),
        ...
        cv2.putText(
            self.im,
            "sensor noise: {:.2g}".format(self.sigma_meas),
            (w - 200, 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            white,
        ...
        cv2.putText(
            self.im, status, (w - 200, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, white, 2
        )
        ...
        scale = (1.0, h / w)  # correct for aspect ratio distortion if h != w
        nsigma = 3.0
        ...
        cv2.putText(
            self.im, "y vs. vy:", (w - 100, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, green, 2
        )
